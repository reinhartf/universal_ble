...
interface UniversalBlePlatformChannel {
  ...
  fun getKnownDevices(withIdentifiers: List<String>, callback: (Result<List<UniversalBleScanResult>>) -> Unit)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: UniversalBlePlatformChannel?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.universal_ble.UniversalBlePlatformChannel.getKnownDevices$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val withIdentifiersArg = args[0] as List<String>
            api.getKnownDevices(withIdentifiersArg) { result: Result<List<UniversalBleScanResult>> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
