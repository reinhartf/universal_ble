...
void UniversalBlePlatformChannel::SetUp(
  ...
  {
    BasicMessageChannel<> channel(binary_messenger, "dev.flutter.pigeon.universal_ble.UniversalBlePlatformChannel.clearGattCache" + prepended_suffix, &GetCodec());
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          const auto& args = std::get<EncodableList>(message);
  ...
  {
    BasicMessageChannel<> channel(binary_messenger, "dev.flutter.pigeon.universal_ble.UniversalBlePlatformChannel.getKnownDevices" + prepended_suffix, &GetCodec());
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          const auto& args = std::get<EncodableList>(message);
          const auto& encodable_with_identifiers_arg = args.at(0);
          if (encodable_with_identifiers_arg.IsNull()) {
            reply(WrapError("with_identifiers_arg unexpectedly null."));
            return;
          }
          const auto& with_identifiers_arg = std::get<EncodableList>(encodable_with_identifiers_arg);
          api->GetKnownDevices(with_identifiers_arg, [reply](ErrorOr<EncodableList>&& output) {
            if (output.has_error()) {
              reply(WrapError(output.error()));
              return;
            }
