...
class UniversalBlePlatformChannelSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: UniversalBlePlatformChannel?, messageChannelSuffix: String = "") {
    ...
    let getKnownDevicesChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.universal_ble.UniversalBlePlatformChannel.getKnownDevices\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      getKnownDevicesChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let withIdentifiersArg = args[0] as! [String]
        api.getKnownDevices(withIdentifiers: withIdentifiersArg) { result in
          switch result {
          case .success(let res):
            reply(wrapResult(res))
          case .failure(let error):
